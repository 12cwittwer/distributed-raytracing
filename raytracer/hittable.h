    ...
    bool hit(const ray& r, interval ray_t, hit_record& rec) const {
        switch (type) {
            case HITTABLE_TRANSLATE:
                return translate->hit(r, ray_t, rec);
...
struct hittable_translate {
    hittable* object;  // Raw pointer to the wrapped hittable
    vec3 offset;
    aabb bbox;
